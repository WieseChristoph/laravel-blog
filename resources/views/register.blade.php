        ...
        <main class="flex justify-center items-center w-full p-5">
            <form method="POST" action="{{ route("register") }}" enctype="multipart/form-data" class="flex flex-col gap-3">
                <div class="flex flex-col">
                    ...
                    <input type="file" name="avatar" id="avatar" accept="image/*" class="text-white">
                    @error("avatar")
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" name="name" id="name" placeholder="Name" class="text-black" value="{{ old("name") }}" required />
                    @error("name")
                        <span class="text-red-600">{{ $message }}</span>
                    @enderror
                </div>
